...
const ALLOW_ENV_RECIPIENT_FALLBACK = String(process.env.ALLOW_ENV_RECIPIENT_FALLBACK || 'false').toLowerCase() === 'true';

let getConfig = () => ({});

// SMTP available?
const smtpEnvReady = !!process.env.SMTP_HOST && !!process.env.SMTP_USER && !!process.env.SMTP_PASS;
const smtp = smtpEnvReady ? nodemailer.createTransport({
...
function setConfigGetter(fn) { if (typeof fn === 'function') getConfig = fn; }
...
async function sendEmail({ subject, text }) {
  ...
  // Never send if globally disabled
  if (!alertsEnabled) {
  ...
  if (!smtpEnvReady || toList.length === 0) {
    console.log("(email dry-run: missing recipients or SMTP)", { subject, toListLen: toList.length, smtpEnvReady });
    return;
  }

  try {
    await smtp.sendMail({
      from: cfg.alert_from_email || process.env.ALERT_FROM_EMAIL || process.env.SMTP_USER,
      to: toList,
      subject,
      text
    });
